...
echo ""
echo "${BLUE}Note: If you get Redis IP allow list errors:${NC}"
echo "  - Use render-no-redis.yaml (deploy without Redis first)"
echo "  - Then manually create Redis service in Render dashboard"
echo "  - Or use render-simple.yaml as alternative"
echo ""
echo "${YELLOW}Manual Redis Setup (if needed):${NC}"
echo "1. Deploy using render-no-redis.yaml"
echo "2. Go to Render dashboard → New + → Redis"
echo "3. Name: vinkit-redis, Plan: Starter"
echo "4. Update backend REDIS_URL environment variable"
echo ""
